...
%\enlargethispage{1cm}
Let us seek the prediction as a function $f(\vec{x})=\alpha_1x_1+\cdots+\alpha_kx_k$ with restriction $f(\vec{x})\leq c$ inside a unit ball $\norm{\vec{x}}_2^2=x_1^2+x_2^2+\cdots+x_k^2\leq 1$. 
...
&\text{s.t. } \alpha_1^2+\cdots+ \alpha_k^2\leq c
\end{align*}
...
&\text{s.t. } \abs{\alpha_1}+\cdots+\abs{\alpha_k}\leq c
\end{align*}
...
\middlefoil{Motivating examples\vspace*{1ex}\\ {\large Unsupervised learning}}
...
\foilhead[-1cm]{Belief propagation for continous distibutions}
