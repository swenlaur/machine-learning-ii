...
\newcommand{\ldate}{11 February, 2019}
...
\illustration[scale=0.8]{biase-variance-dilemma}
\vspace*{-.5cm}
...
\foilhead[-1cm]{Different flavours of cross validation}
...
\textbf{Direct estimation of model error}
\begin{triangles}
\item The model error is estimated as an average of test errors $E_i$.
\item If needed one computes standard deviation estimates of the average error.  
\item If needed one draws a boxplot of all test errors.  
\end{triangles}
\vspace*{1cm}

\textbf{Model error through the optimism}
\begin{triangles}
\item First the optimism $\Delta$ is estimated as an average  $E_i-S_i$.
\item Next model is trained on the entire data and training error $S$ is recorded.  
\item Model error is estimated as $S+\Delta$.
\item If needed the standard deviation is computed for the optimism.  
\item If needed one draws a shifted boxplot $S+E_i-S_i$ for the error estimate.  
\end{triangles}
